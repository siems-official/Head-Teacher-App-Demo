...
const DueListSectionScreen = () => {
  const params = useLocalSearchParams();
  const router = useRouter();
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const { selectedInvoiceYear, pendingCollectionClassSectionWise } =
  ...
  const classBasedFilteredInvoices = pendingCollectionClassSectionWise?.filter(
        ...
        <Fragment key={keyIndex}>
          {key === "section" ? (
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  pathname: routes.accounts.subRoutes.dueListStudent.path,
                  params: item,
                })
              }
              activeOpacity={0.35}
...
export default DueListSectionScreen;
