...
const DueListStudentScreen = () => {
  const params = useLocalSearchParams();
  const router = useRouter();
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const { selectedInvoiceYear, pendingCollectionClassSectionStudentWise } =
  ...
  const [selectedDueItem, setSelectedDueItem] = useState(null);
  ...
  const classSectionBasedFilteredInvoices =
    pendingCollectionClassSectionStudentWise?.filter(
      (item) =>
          ...
          {key === "student_name" ? (
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  pathname:
                    routes.accounts.subRoutes.dueListStudent.subRoutes.details
                      .path,
                  params: item,
                })
              }
...
export default DueListStudentScreen;
