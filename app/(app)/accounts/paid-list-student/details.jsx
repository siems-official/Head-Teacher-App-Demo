...
const PaidListStudentDetailsScreen = () => {
  const { width } = useWindowDimensions();
  const params = useLocalSearchParams();
  const { user } = useSelector((state) => state.auth);
  const { studentSpecificPaidInvoices } = useSelector((state) => state.invoice);
  ...
  const renderItem = ({ item, index }) => (
    <View
      className={cn(
        ...
        index ===
          studentSpecificPaidInvoices[params?.student_username]?.length - 1
          ? "border-b-0"
          : "border-b"
      )}
    >
  ...
  return (
    <BaseLayout title="Paid List">
      <SelectItemPicker
        selected={selectedPaidItem}
        setSelected={setSelectedPaidItem}
        placeholder="Select"
        options={[{ label: "Paid List - 2025", value: "paid list - 2025" }]}
      ...
      <DataTable
        ...
        data={studentSpecificPaidInvoices[params?.student_username]}
...
export default PaidListStudentDetailsScreen;
