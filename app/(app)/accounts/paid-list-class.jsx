...
const PaidListClassScreen = () => {
  const router = useRouter();
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const { selectedInvoiceYear, paidCollectionClassWise } = useSelector(
    (state) => state.invoice
  );
  ...
  const renderItem = ({ item, index }) => (
    <View
      className={cn(
        ...
        index === paidCollectionClassWise?.length - 1
          ? "border-b-0"
          : "border-b"
      )}
    >
        ...
        <Fragment key={keyIndex}>
          {key === "class" ? (
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  pathname: routes.accounts.subRoutes.paidListSection.path,
                  params: item,
                })
              }
              activeOpacity={0.35}
...
export default PaidListClassScreen;
