...
const ApprovedApplicationsTeachersScreen = () => {
  ...
  const handleGetLeaves = () => {
    ...
    getFilteredLeaves({
      ...
      status: enumList.applicationStatus.APPROVED,
      type: enumList.userType.TEACHER,
  ...
  const renderItem = ({ item, index }) => (
    <View
      className={cn(
        ...
        index === approvedteacherStuffApplications?.data?.length - 1
          ? "border-b-0"
          : "border-b"
      )}
    >
          ...
          {key === "action" ? (
            <TouchableOpacity
              ...
              onPress={() =>
                router.push({
                  pathname:
                    routes.leaveManagement.subRoutes.approvedApplications
  ...
  return (
    <BaseLayout title="Approved Applications">
      <View className="relative flex-1">
        ...
        <DataTable
          ...
          data={approvedteacherStuffApplications?.data}
          ...
          noDataMessage="Enter Teacher ID or Date to search"
        />
      </View>
    </BaseLayout>
  );
...
export default ApprovedApplicationsTeachersScreen;
