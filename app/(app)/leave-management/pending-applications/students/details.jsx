...
import { cn, colors, FONTS, secondTimestampToDate } from "@/services";
...
const PendingApplicationsStudentsDetailsScreen = () => {
  ...
  const { pendingStudentApplications } = useSelector(
  ...
  const applicationDetails = pendingStudentApplications?.reservedData?.find(
  ...
  const detailedData = [
    {
      label: "Name",
      value: applicationDetails?.student_id?.name_english || "N/A",
    },
    {
      label: "Class",
      value:
        applicationDetails?.student_id?.class_id?.local_class_name || "N/A",
    },
    {
      label: "Section",
      value: applicationDetails?.student_id?.section_id?.section_name || "N/A",
    },
    {
      label: "Roll",
      value: user?.student?.current_roll_number || "N/A",
    },
    {
      label: "ID",
      value: applicationDetails?.student_id?.username || "N/A",
    },
    {
      label: "Contact",
      value: applicationDetails?.student_id?.mobile_number || "N/A",
    },
    {
      ...
      <View className="border border-neutral-300 p-4 mx-4 mt-2 rounded-xl flex-col gap-2">
        {detailedData?.map((item, index) => (
          <View className="flex-row justify-between gap-4" key={index}>
            ...
            <Text
              className={cn("!leading-[1.2] max-w-[160px] text-right")}
              style={{
                ...FONTS.inter600,
                fontSize: 14,
                color:
...
export default PendingApplicationsStudentsDetailsScreen;
