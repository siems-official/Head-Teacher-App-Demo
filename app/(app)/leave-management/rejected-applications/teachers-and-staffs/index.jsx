...
const RejectedApplicationsTeachersScreen = () => {
  const router = useRouter();
  const { user } = useSelector((state) => state.auth);
  const { selectedYear } = useSelector((state) => state.academicYear);
  const { rejectedteacherStuffApplications } = useSelector(
  ...
  const renderItem = ({ item, index }) => (
    <View
      className={cn(
        ...
        index === rejectedteacherStuffApplications?.data?.length - 1
          ? "border-b-0"
          : "border-b"
      )}
    >
    ...
    <BaseLayout title="Rejected Applications">
      <View className="relative flex-1">
        ...
        <DataTable
          ...
          data={rejectedteacherStuffApplications?.data}
...
export default RejectedApplicationsTeachersScreen;
