...
const AbsentScreen = () => {
  ...
  const { absentStudentList } = useSelector((state) => state.studentAttendance);
  ...
  const classSpecificAbsentStudentList = absentStudentList?.filter(
    (student) =>
  ...
  const renderItem = ({ item, index }) => (
    <View
      className={cn(
        ...
        index === classSpecificAbsentStudentList.length - 1
          ? "border-b-0"
          : "border-b"
      )}
    >
  ...
  return (
    <BaseLayout title="Absent">
      ...
      <DataTable
        ...
        data={classSpecificAbsentStudentList}
